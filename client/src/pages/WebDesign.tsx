  ...
  return (
    ...
    <div className="min-h-screen bg-white">
              ...
              >
                <div className="overflow-hidden mb-4 bg-gray-200 relative aspect-[4/3]">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="bg-primary text-[#1A1A1A] px-4 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
      ...
      <Dialog open={!!selectedProject} onOpenChange={(open) => !open && setSelectedProject(null)}>
        <DialogContent className="max-w-[95vw] w-full h-[90vh] p-0 bg-black border-none rounded-xl overflow-hidden [&>button]:hidden">
          ...
          <div className="w-full h-full flex flex-col">
            <div className="bg-black text-[#1A1A1A] px-4 py-3 flex items-center justify-between border-b border-[#1A1A1A]/10">
              <div className="flex items-center gap-3">
                <h3 className="font-display uppercase text-lg">{selectedProject?.title}</h3>
                <a href={selectedProject?.url} target="_blank" rel="noopener noreferrer" className="text-xs text-[#2A2A2A]/60 hover:text-primary flex items-center gap-1">
                  Open in new tab <ExternalLink className="w-3 h-3" />
                </a>
              </div>
              <DialogClose asChild>
                <Button variant="ghost" size="icon" className="text-[#1A1A1A] hover:bg-[#1A1A1A]/5 rounded-full h-8 w-8">
                  <X className="w-4 h-4" />
                </Button>
              </DialogClose>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service, index) => {
              ...
              return (
                <div 
                  key={index}
                  className="group relative p-8 bg-white hover:bg-[#D9A3B0] transition-colors duration-300"
                >
                  ...
                  <p className="text-sm leading-relaxed text-black/60 group-hover:text-[#2A2A2A]/70 transition-colors">
                    {service.description}
                  </p>
                </div>
              );
      ...
      <section className="py-20 bg-[#1a1a1a]">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl md:text-5xl font-display text-[#1A1A1A] uppercase leading-[0.9] mb-4 text-center">
            Our Design Process
          </h2>
          <p className="text-[#2A2A2A]/60 text-center max-w-2xl mx-auto mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {processSteps.map((step, index) => (
              <div key={index} className="relative">
                ...
                <h3 className="text-lg font-display uppercase text-[#1A1A1A] mb-3">{step.title}</h3>
                <p className="text-sm text-[#2A2A2A]/60 leading-relaxed">{step.description}</p>
              </div>
            ))}
          </div>
        </div>
