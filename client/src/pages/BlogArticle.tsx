...
function renderMarkdownLinks(text: string) {
  ...
  return parts.map((part, i) => {
    ...
    if (linkMatch) {
      ...
      if (isExternal) {
        return (
          <a key={i} href={linkMatch[2]} target="_blank" rel="noopener noreferrer" className="text-[#D9A3B0] font-bold hover:underline">
            {linkMatch[1]}
          </a>
        );
      }
      return (
        <Link key={i} href={linkMatch[2]} className="text-[#D9A3B0] font-bold hover:underline">
          {linkMatch[1]}
        </Link>
      );
    }
...
function SectionRenderer({ section }: { section: ArticleSection }) {
  ...
  switch (section.type) {
    ...
    case "blockquote":
      return (
        <blockquote className="my-10 border-l-4 border-primary bg-[#A995D6]/5 py-6 px-8 relative">
    ...
    case "stat":
      return (
        <div className="my-10 bg-[#D9A3B0] text-[#1A1A1A] p-8 md:p-10 text-center">
          ...
          <p className="text-[#2A2A2A]/70 max-w-lg mx-auto">{section.description}</p>
        </div>
      );
      ...
      return (
        <div className="my-10 grid grid-cols-2 md:grid-cols-3 gap-4">
          {section.stats.map((stat, i) => (
            <div key={i} className="bg-black text-[#1A1A1A] p-6 text-center">
              <div className="text-3xl md:text-4xl font-display text-primary mb-2">{stat.value}</div>
              <div className="text-sm text-[#2A2A2A]/70 uppercase tracking-wider">{stat.label}</div>
            </div>
          ))}
        </div>
      );
    ...
    case "callout":
      const variants = {
        ...
        tip: "bg-[#A995D6]/10 border-primary/30 text-black",
    ...
    case "cta":
      ...
      return (
        <div className="my-12 bg-[#D9A3B0] p-8 md:p-12 text-center">
          <p className="text-xl md:text-2xl font-display uppercase text-[#1A1A1A] mb-6">{section.text}</p>
          {isExternal ? (
            <a
              href={section.linkHref}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-primary text-[#1A1A1A] font-semibold px-8 py-4 text-lg hover:bg-primary/90 transition-colors uppercase tracking-wider"
          ...
          <div className="absolute inset-0 flex items-end">
            <div className="container mx-auto px-4 pb-12 md:pb-16">
              <div className="max-w-3xl">
                ...
                <h1 className="text-3xl md:text-5xl lg:text-6xl font-display uppercase text-[#1A1A1A] leading-tight mb-4">
                  {article.title}
                </h1>
                <div className="flex items-center gap-6 text-[#2A2A2A]/60 text-sm">
        ...
        {relatedArticles.length > 0 && (
          <section className="bg-black py-16 md:py-20">
            <div className="container mx-auto px-4">
              <h2 className="text-3xl md:text-4xl font-display uppercase text-[#1A1A1A] text-center mb-12">
                Keep Reading
              </h2>
                ...
                {relatedArticles.map((related) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group block bg-[#1A1A1A]/5 border border-[#1A1A1A]/10 hover:border-primary/50 transition-all"
                    ...
                    >
                      ...
                      <div className="p-6">
                        ...
                        <h3 className="text-lg font-display uppercase text-[#1A1A1A] group-hover:text-primary transition-colors mb-2">
                          {related.title}
                        </h3>
                        <p className="text-[#2A2A2A]/60 text-sm line-clamp-2">{related.excerpt}</p>
                      </div>
                    </Link>
                  );
                })}
