    ...
    <section id="services" className="bg-[#121212] py-24 relative overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <motion.div 
          ...
          className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8 border-b border-[#1A1A1A]/10 pb-8"
        >
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-primary text-xl">✦</span>
              <span className="text-[#2A2A2A]/60 uppercase tracking-widest text-xs font-bold">What We Do</span>
            </div>
            <h2 className="text-4xl md:text-6xl font-display text-[#1A1A1A] uppercase max-w-3xl leading-[0.9]">
        ...
        >
          {services.map((service, index) => {
            ...
            <Component 
              key={index}
              href={service.link}
              className={`group relative border ${service.highlight ? 'border-primary bg-primary' : 'border-[#1A1A1A]/10 bg-[#1A1A1A]/5'} p-8 min-h-[400px] flex flex-col justify-between hover:translate-y-[-8px] transition-transform duration-300 block`}
            >
              ...
              <p className={`text-sm leading-relaxed ${service.highlight ? 'text-black/80' : 'text-[#2A2A2A]/60'}`}>
                {service.description}
              </p>
            </Component>
          )})}
        ...
        <div className="flex justify-center mt-12">
          <Link 
            ...
            className="group px-8 py-4 bg-primary text-[#1A1A1A] border border-primary hover:bg-white hover:text-black hover:border-[#1A1A1A] transition-all text-xs font-bold uppercase tracking-widest flex items-center gap-3"
          >
